...
Mmc::Mmc(UserInput user_input,Lut& lut, int polarity)
    : id_(++id_count_),
      ...
      polarity_(polarity),
      clock_rate_(user_input.clock_rate){
  this->time_step_ = lut_.get_time_step();
  this->num_points_ =
      static_cast<int>(this->total_time_ / this->clock_rate_) + 1;
...
std::vector<double> Mmc::generate_pulses_clock() {
  generate_time_points_clock();
  generate_trigger_times1();
  generate_pulse_shapes();
  add_noise();
  return signal_;
...
void Mmc::generate_time_points_clock() {
  time_points_.resize(num_points_);
  for (int i = 0; i < num_points_; i++) {
    time_points_[i] = i * clock_rate_;
  }
}
